...
function preload ()
{
	...
	//Hero
	this.load.atlas("hero", "../assets/sprites/images/hero-sheet.png", "../assets/sprites/hero.json");
	this.load.atlas("monster", "../assets/sprites/images/monster-sheet.png", "../assets/sprites/monster.json");
	this.load.atlas("pentagramme", "../assets/sprites/images/pentagramme.png", "../assets/sprites/pentagramme.json");
}
...
function create ()
{
	...
	var layer1 = map.createDynamicLayer('decor', tileset, 32, 32);
	var layer2 = map.createDynamicLayer('mur', tileset, 32, 32);
	var layer3 = map.createStaticLayer('chemins', tileset, 32, 32);
	var layer4 = map.createStaticLayer('pnj', tileset, 32, 32);
	var layer5 = map.createDynamicLayer('mur_invisible_sans_porte', tileset, 32, 32); 
	var layer6 = map.createStaticLayer('porte_invisible', tileset, 32, 32); 
...
function genereEnnemis(scene, cibleMonstre)
{
	...
	for (let i = 0; i < nombreDePentagramme; i++) 
	{
		...
		let temp = new Pentagramme(scene, coordonneesPentagrammeX[i], coordonneesPentagrammeY[i], 5, 'pentagramme', nomPentagramme, 32, 32);
		scene.add.existing(temp);
		pentagrammes.set(nomPentagramme, temp);
	}
			
	genereMonstres(scene, 0, nombreDeMonstresParPentagramme, delaiVagueMonstre, cibleMonstre)		
}
	
	
function genereMonstres(scene, numIteration,  nombreDeMonstresParPentagramme, delaiVagueMonstre, cibleMonstre)
{
		let numeroMonstre = 0;
		...
		for (let p of pentagrammes.values()) 
		{
			...
			var creationMonstre = function (posPentagramme)
				 {
					...
					let temp = new Monster(scene, posPentagramme.x, posPentagramme.y, 50, 'monster', idMonstre, 32, 32, cibleMonstre);
					let layerTemp = mapLayers.get('mur_invisible_sans_porte');
					
					monstres.set(idMonstre, temp);
					
					scene.physics.add.collider(temp, layerTemp, function(){temp.eviteObstacle()}, null, null);
					
					scene.physics.add.collider(temp, sorciere, function(){temp.eviteObstacle()}, null, null);
			...
			setTimeout(function(){creationMonstre(p);}, departAleatoireDesMonstres);
		...
		{
			setTimeout(function(){genereMonstres(scene, numIteration, nombreDeMonstresParPentagramme, delaiVagueMonstre, cibleMonstre)}, delaiVagueMonstre+departAleatoireDesMonstres); 
		}
}
